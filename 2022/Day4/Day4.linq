...
public static IEnumerable<int> GetNumberListFor(string[] section)
{
	...
	//Enumerable.Range(start, end - start + 1).Dump($"section {section[0]} - {section[1]}");
...
public static async Task<long> Part1(int dayNumber)
{
	...
	return inputList.Select(i => 
	{
		...
		var isSubset = firstPairRange.Intersect(secondPairRange);
		var isFullyContained = isSubset.Count() == firstPairRange.Count() || isSubset.Count() == secondPairRange.Count();
...
public static async Task<long> Part2(int dayNumber)
{
	...
	return inputList.Select(i =>
	{
		...
		var secondPairRange = GetNumberListFor(sectionPair[1]);
		var isSubset = firstPairRange.Intersect(secondPairRange);
		var isOverlap = isSubset.Count() > 0;
