...
public static async Task<long> Part1()
{
	...
	Dictionary<(char Opponent, char Me), int> combinationScore =
	...
	Dictionary<char, int> shapeScore = new Dictionary<char, int> {
		{'X', 1},
		{'Y', 2},
		{'Z', 3}
	};
	foreach (var row in inputList)
	{
		...
		{
			//combinationScore[round].Dump();
			score += combinationScore[round] + shapeScore[row[2]];
		}
	}
	return score;
